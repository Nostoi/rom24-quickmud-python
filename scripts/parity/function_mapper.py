    ...
    for c_file in src_dir.glob("*.c"):
        try:
            ...
            for match in func_pattern.finditer(content):
                ...
                functions[c_file.stem].append(
                    FunctionInfo(name=func_name, file=c_file.stem, line=line_num, signature=match.group(0))
                )
    ...
    for py_file in mud_dir.rglob("*.py"):
        ...
        try:
            ...
            for match in re.finditer(r"^(?:    )?def (\w+)\(", content, re.MULTILINE):
                ...
                functions[func_name] = FunctionInfo(name=func_name, file=str(rel_path), line=line_num)
        except Exception as e:
...
def generate_mapping_report(
    c_functions: dict[str, list[FunctionInfo]], py_functions: dict[str, FunctionInfo], output_path: Path
) -> None:
