...
import pytest
from mud.combat.engine import apply_damage_reduction, is_good, is_evil, is_neutral
...
def test_alignment_classification_functions():
    ...
    assert is_good(good_char) == True
    assert is_good(very_good_char) == True
    assert is_evil(good_char) == False
    assert is_neutral(good_char) == False
    
    # Test evil alignment (<= -350)
    evil_char = Character(alignment=-350)
    very_evil_char = Character(alignment=-1000)
    assert is_evil(evil_char) == True
    assert is_evil(very_evil_char) == True
    assert is_good(evil_char) == False
    assert is_neutral(evil_char) == False
    ...
    assert is_neutral(neutral_char) == True
    assert is_neutral(slightly_good_char) == True  
    assert is_neutral(slightly_evil_char) == True
    assert is_good(neutral_char) == False
    assert is_evil(neutral_char) == False
