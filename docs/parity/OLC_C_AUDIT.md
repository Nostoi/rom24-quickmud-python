...
| `do_alist` | 1478-1502 | public | List all areas with vnum range / filename / security / builders, header row included | `mud/commands/imm_olc.py:do_alist:121-146` — iterates `getattr(registry, "areas", [])` but `registry.areas` does not exist (the registry exports `area_registry` dict). Returns header + nothing on a live system. **Real bug, not just gap.** | ⚠️ BROKEN (OLC-023) |
...
| `OLC-023` | CRITICAL | `src/olc.c:1478-1502` | `mud/commands/imm_olc.py:121-146` | `do_alist` iterates the wrong attribute (`registry.areas` does not exist; should be `registry.area_registry.values()`). Returns header-only output on a live system. **Real bug.** | 🔄 OPEN |
