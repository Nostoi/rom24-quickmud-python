...
| `OLC-006` | IMPORTANT | `src/olc.c:216-238` | `mud/commands/build.py:_interpret_aedit` | All 14 active `aedit_table[]` entries implemented inline in `_interpret_aedit` (`age`, `builder`, `commands`, `create`, `filename`, `name`, `reset`, `security`, `show`, `vnum`, `lvnum`, `uvnum`, `credits`, flag-toggle prefix). `aedit_age` (`src/olc_act.c:770-790`) was the only missing entry — added: sets `pArea->age`, no `changed` flag (ROM parity), syntax error on non-numeric/empty. 6 tests added to `test_olc_011_aedit_flag_toggle.py`. Data-driven table refactor deferred (structural, not a parity gap). | ✅ FIXED |
...
| `OLC-012` | MINOR | `src/olc.c:474-527` | `mud/commands/dispatcher.py:_should_fallback_from_olc` | `redit` fallback to `interpret(ch, arg)` is implemented: `_interpret_redit` returns `"Unknown room editor command."` → `_should_fallback_from_olc` returns `True` → `_process_descriptor_input` returns `None` → `process_command` re-dispatches full input via normal table. Verified by 14 integration tests. | ✅ FIXED |
| `OLC-013` | MINOR | `src/olc.c:532-584` | `mud/commands/dispatcher.py:_should_fallback_from_olc` | `oedit` fallback: same mechanism as OLC-012 (`"Unknown object editor command: {cmd}"` sentinel). Verified by integration tests. | ✅ FIXED |
| `OLC-014` | MINOR | `src/olc.c:589-641` | `mud/commands/dispatcher.py:_should_fallback_from_olc` | `medit` fallback: same mechanism as OLC-012 (`"Unknown mobile editor command: {cmd}"` sentinel). Verified by integration tests. | ✅ FIXED |
