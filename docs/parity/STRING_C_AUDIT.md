...
- **Status**: ✅ AUDITED 100% — 12 of 12 helpers FIXED (STRING-001/002/003/004/005/006/007/008/009/010/011/012).
- **Date**: 2026-04-29
...
| `string_add` | 121-286 | public | Editor input dispatcher (`.c`/`.s`/`.r`/`.f`/`.h`/`.ld`/`.li`/`.lr`/`~`/`@`); MPCODE save hook | `mud/utils/string_editor.py:string_add` | ✅ FIXED (STRING-004) |
| `format_string` | 299-451 | public | Word-wrap to 77 cols + sentence capitalization + paren/quote handling | `mud/utils/string_editor.py:format_string` | ✅ FIXED (STRING-005) |
...
| `STRING-004` | CRITICAL | `src/string.c:121-286` | `string_add` editor dispatcher — reads `.c/.s/.r/.f/.ld/.li/.lr/.h` dot-commands, `~`/`@` to terminate, MAX_STRING_LENGTH-4 truncation, `smash_tilde` on every line, MPCODE post-save hook (writes back to `mob_index_hash` mprogs). The whole editor UX. | ✅ FIXED — `mud/utils/string_editor.py:string_add(session, raw_line) -> str \| None`. All 8 dot-commands implemented. Terminator checked before smash_tilde (pragmatic deviation: ROM's order makes `~` dead code; both `~` and `@` work). Length cap `len(buffer)+len(line) >= max_length` forces editor exit without `on_commit` (mirrors ROM line 271). MPCODE mprog write-back is responsibility of the `on_commit` callback registered by the OLC layer. Test: `tests/integration/test_string_editor_string_add.py` (24 cases). |
| `STRING-005` | IMPORTANT | `src/string.c:299-451` | `format_string` — sentence-end double-space, capitalize-after-`.`/`?`/`!`, balanced paren rewrite, 77-col word-wrap with `bug("No spaces", 0)` mid-word `-` fallback. Cited by `.f` dot-command and many `olc_act.c` `desc` builders. | ✅ FIXED — `mud/utils/string_editor.py:format_string(string) -> str`. Two-phase ROM port: phase-1 tokenization (space collapse, sentence double-space, quote-after-punct, paren rebalance, capitalization); phase-2 77-col wrap (first-wrap scans from col 73, subsequent from 76; mid-word `-\n\r` fallback with `logging.getLogger` warning). Test: `tests/integration/test_string_editor_format_string.py` (16 cases). |
...
| `STRING-009` | IMPORTANT | `src/string.c:574-605` | `string_linedel(string, line)` — remove 1-indexed line, preserving `\n\r` line-ending. `.ld` dot-command. | ✅ FIXED — `mud/utils/string_editor.py:string_linedel(string, line) -> str`. Removes 1-indexed line; out-of-range is no-op. Test: `tests/integration/test_string_editor_linedel.py` (8 cases). |
| `STRING-010` | IMPORTANT | `src/string.c:607-645` | `string_lineadd(string, newstr, line)` — insert `newstr` as 1-indexed line. `.li`/`.lr` dot-commands. | ✅ FIXED — `mud/utils/string_editor.py:string_lineadd(string, newstr, line) -> str`. Inserts 1-indexed line; new line gets `\n\r` suffix. Test: `tests/integration/test_string_editor_lineadd.py` (10 cases). |
...
STRING-004 closed 2026-04-29. ROM deviations documented:

1. **`~` terminator ordering** — ROM `src/string.c:128` smashes tildes before the terminator check at line 230, which makes `~` dead code at runtime (it becomes `-`). Python port checks terminator first so both `~` and `@` work as documented in `.h` help text. Comment in `string_add` cites both ROM line numbers.
2. **MPCODE write-back** — ROM lines 232-251 walk `mob_index_hash` to sync mprog source on `ED_MPCODE` terminate. Python delegates this to `session.string_edit.on_commit` callback registered by the OLC layer; the dispatcher itself only clears state.

## Phase 5 — Completion

`string.c` is now ✅ AUDITED 100% — all 12 public functions ported and tested. The final gap (STRING-004) closed 2026-04-29 with 24 integration test cases covering all dot-commands, both terminators, length cap enforcement, on_commit callback, and multi-word `.li`/`.lr` arguments.
