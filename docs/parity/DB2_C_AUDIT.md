...
|---|---|---|---|---|---|
| **DB2-001** | CRITICAL | `src/db2.c:239` | `mud/loaders/mob_loader.py:118` | `load_mobiles` does not OR `ACT_IS_NPC` into prototype `act_flags`. ROM unconditionally adds it; Python relies on the area file always including the `A` letter. Mobs missing the letter get spawned with `is_npc=False` semantics in flag tests. | 🔄 OPEN |
| **DB2-002** | CRITICAL | `src/db2.c:239–242,279–286,295–297` | `mud/loaders/mob_loader.py:118–119,92–95,106–107` | `load_mobiles` does not merge `race_table[race].{act,aff,off,imm,res,vuln,form,parts}` into the prototype's flag fields. Race-derived defaults (e.g., dragon flying, vampire immunities, undead form) are silently dropped at load time. `MobInstance.from_prototype` (`templates.py:298–303`) reads the prototype as-is, so the loss propagates to every spawn. | 🔄 OPEN |
| **DB2-003** | IMPORTANT | `src/db2.c:236–237` | `mud/loaders/mob_loader.py:62–63` | `load_mobiles` does not uppercase the first character of `long_descr` and `description`. ROM does `pMobIndex->long_descr[0] = UPPER(...)` to defend against area-builder typos; Python preserves verbatim, so any mob with a lowercase first letter in those fields renders that way in the room/look output. | 🔄 OPEN |
