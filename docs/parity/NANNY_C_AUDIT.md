...
| `NANNY-004` | CRITICAL | `nanny.c:653` | `mud/models/character.py:1047-1051` | `CON_PICK_WEAPON` does not set `learned[weapon_gsn] = 40` — chosen weapon proficiency is 0%. | ✅ VERIFIED — false positive: `from_orm` seeds the picked weapon's skill to ≥40 on every load via `_STARTING_WEAPON_SKILL_BY_VNUM` lookup of `default_weapon_vnum` (`mud/models/character.py:1047-1051`). Audit had cited the prompt path; the seed lives on the load side. Regression test: `tests/integration/test_nanny_login_parity.py::test_chosen_weapon_skill_seeded_at_40_percent`. |
