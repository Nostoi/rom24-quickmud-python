...
| `CON_READ_MOTD` (entry broadcast) | 804-815 | `broadcast_entry_to_room` | `mud/net/connection.py` | ✅ AUDITED — player/pet room-entry broadcasts closed by NANNY-007/008, and May 21 trust-rebuild revalidated the tail of the branch so login now also emits the ROM board summary after the initial auto-look. |
