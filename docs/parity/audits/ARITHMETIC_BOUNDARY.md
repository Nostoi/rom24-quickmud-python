...
| ❌ MISSING — open | 41 | 19% |
| ✅ FIXED post-triage | 3 (ARITH-010, ARITH-015, ARITH-016) | 1% |
...
| ARITH-024 | `mud/groups/xp.py:117-122` | `group_gain` gates the "You receive %d experience points." message and `gain_exp` call on `if xp <= 0: continue`. ROM `src/fight.c:1786-1789` is unconditional — when `xp == 0` (reachable when `level_range < -9` or outside the base_exp table), ROM prints "You receive 0 experience points." and calls `gain_exp(gch, 0)`. Python suppresses both. Surfaced 2026-05-27 during ARITH-009 reclass analysis. |
...
| 55b | mud/groups/xp.py:117-122 | `if xp <= 0: continue` | src/fight.c:1786-1789 — unconditional message + gain_exp | No | ❌ MISSING | ARITH-024 | Python silently drops the "You receive 0 experience points." message and skips `gain_exp(gch, 0)` when `xp_compute` returns 0 (reachable when `level_range < -9`). ROM is unconditional. |
