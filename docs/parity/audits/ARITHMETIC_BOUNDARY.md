...
| 26 | mud/commands/shop.py:822 | `max(0, unit_price - discount)` | src/act_obj.c:2722–2729 do_buy | No | ❌ MISSING | ARITH-111 | ROM item-shop haggle does `cost -= obj->cost/2 * roll/100` raw — but unlike the pet-shop branch (ARITH-110), the discount here is derived from `obj->cost` (the unmarked-up proto cost) and applied to the marked-up `cost = obj->cost * profit_buy / 100`. **Reachable** when `profit_buy < 50` (i.e., shop sells below half cost — possible in custom areas), because then max discount `≈ proto.cost / 2` can exceed `unit_price < proto.cost / 2`. Python clamps the negative to 0 (free item); ROM lets `cost` go negative, and `deduct_cost(ch, cost * number)` with negative cost interacts with the player's gold/silver split in ways that need careful audit. Hold-back: a clean close requires understanding the deduct_cost-side semantics, not just dropping the floor. Probe-then-scope candidate for a future session. |
