...
|---|-----------|-------|-------------------|------------|--------|--------------------|
| 1 | `_send_to_char` | ⚠️ **PARTIAL FIX** (2.9.24) — canonical `send_to_char_buffered` at `mud/utils/messaging.py`; `conditions.py` migrated. 12 sites still drift: `game_loop.py:341`, `combat/assist.py:196`, `commands/remaining_rom.py:579`, `commands/imm_load.py:398`, `commands/imm_emote.py:222`, `commands/admin_commands.py:173`, `commands/imm_punish.py:250`, `commands/imm_server.py:251`, `commands/imm_admin.py:324`, `commands/imm_commands.py:483`, `commands/imm_display.py:296`, `skills/handlers.py:463`. | mixed — `game_loop.py` correct (async + mailbox); `conditions.py` was messages-only (✅ fixed); **9 imm_*/admin/remaining_rom write to a nonexistent `char.output_buffer`** (never delivered to anyone — fresh bug class surfaced by audit re-read); `combat/assist.py` + `skills/handlers.py` duplicate-deliver (async + mailbox both fire). | `conditions.py` writes ONLY to `messages` list — connected players never receive thirst/hunger messages, only the test buffer. **9 imm_*/admin/remaining_rom copies write to `char.output_buffer`** — an attribute the production read loop never drains. Every message vanishes. `combat/assist.py` and `skills/handlers.py` write to BOTH async socket AND `messages` (DUPL-002 class duplicate-delivery). | DUPL-001 | ✅ `conditions.py` migrated to canonical. ⚠️ Remaining 12 sites tracked as DUPL-001a (output_buffer black-hole — 9 sites), DUPL-001b (duplicate-delivery — 2 sites: combat/assist + skills/handlers), DUPL-001c (game_loop canonical-equivalent — collapse for consistency). Burn down in follow-up commits. Regression test for the conditions fix: `tests/integration/test_gain_condition_delivers_to_connected_pc.py`. |
