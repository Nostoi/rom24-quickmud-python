...
|---|-----------|-------|-------------------|------------|--------|--------------------|
| 1 | `_send_to_char` | `mud/game_loop.py:341`, `mud/combat/assist.py:196`, `mud/characters/conditions.py:10`, `mud/commands/remaining_rom.py:579`, `mud/commands/imm_load.py:398`, `mud/commands/imm_emote.py:222`, `mud/commands/admin_commands.py:173`, `mud/commands/imm_punish.py:250`, `mud/commands/imm_server.py:251`, `mud/commands/imm_admin.py:324`, `mud/commands/imm_commands.py:483`, `mud/commands/imm_display.py:296`, `mud/skills/handlers.py:463` (13 copies) | mixed — `game_loop.py` correct; `conditions.py` messages-only fallback | `game_loop.py` writes to both async socket + `messages` list (correct per `docs/divergences/MESSAGE_DELIVERY.md`). `conditions.py` writes ONLY to `messages` list — connected players never receive thirst/hunger messages, only the test buffer. AGENTS.md explicitly forbids messages-list-only delivery. Other 11 copies are unverified individually; pattern suggests several are silently broken. | DUPL-001 | Move to `mud/utils/messaging.py:send_to_char_buffered` (or similar). Replace every `_send_to_char` callsite with the canonical import. Audit each conditions.py / imm_*.py callsite for delivery-side bugs. |
| 2 | `_push_message` | `mud/combat/engine.py:197`, `mud/magic/effects.py:144` (2 copies) | both reached | `combat/engine.py` writes to async socket and `return`s — single delivery channel per `docs/divergences/MESSAGE_DELIVERY.md` (avoiding the documented duplicate-delivery bug where the read loop drains `messages` after every command). `magic/effects.py` writes to BOTH async socket AND `messages` list — every magic effect message replays on the next prompt for connected PCs. | DUPL-002 | Move canonical `_push_message` from `combat/engine.py` to `mud/utils/messaging.py`. Delete `magic/effects.py` copy. Add regression test exercising magic effect message single-delivery to a connected character. |
| 3 | `_extract_obj` | `mud/game_loop.py:1001`, `mud/commands/obj_manipulation.py:664`, `mud/commands/imm_load.py:373` (3 copies) | mixed — `game_loop.py` for tick extracts; `obj_manipulation.py` for quaff/drink | `game_loop.py` is recursive (extracts container contents) + uses ROM-canonical `in_room`/`carried_by`/`in_obj` fields. `obj_manipulation.py` is non-recursive AND uses `char.carrying` (wrong attribute per AGENTS.md — must be `char.inventory`). Container contents leak when a quaffed/drunken potion-in-container is extracted. | DUPL-003 | Consolidate to canonical `mud/handler.py:extract_obj` (or similar). Replace all three local copies. Add regression test for recursive extraction through a container. |
| 4 | `_check_improve` | `mud/commands/remaining_rom.py:574`, `mud/commands/thief_skills.py:361`, `mud/commands/misc_player.py:271` (3 copies, ALL `pass`) | all 3 reached | All three are `pass` stubs. Canonical `mud/skills/registry.py:check_improve` exists and is ROM-faithful (skill-improvement-by-use). Three sets of command surfaces (remaining_rom, thief_skills, misc_player) silently skip skill improvement entirely. Players never learn skills via use in those command paths. | DUPL-004 | Delete all three local stubs. Replace callsites with `from mud.skills.registry import check_improve`. Add integration test exercising a thief skill (e.g. `backstab`) and asserting `pcdata.learned[skill]` increased after success. |
