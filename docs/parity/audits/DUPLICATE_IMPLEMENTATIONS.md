...
**Status**: **CLOSED** as of 2.9.33 (2026-05-26). 67 candidates discovered, 5 closed by [2.9.21 `rom_api.py` deletion](#closed-by-rom_apipy-deletion). Final outcomes: **7 ❌ real bugs ✅ FIXED** (DUPL-001×4 + DUPL-002 + DUPL-003 + DUPL-004 + DUPL-005 + DUPL-007 reclassified at fix-time + DUPL-018 refiled-then-closed as FOLLOW-001/002), **1 ⚠️ DEAD-CODE ✅ FIXED** (DUPL-006), **1 reclassified to ✅ MATCH as intentional immortal-bypass** (DUPL-008), **3 ⚠️ CLEANUP ✅ FIXED** (DUPL-010, 011, 019 — genuinely mechanical with canonical method/file created), **9 ⚠️ CLEANUP reclassified to ✅ MATCH** (DUPL-009, 012, 013, 014, 015, 016, 017, 020, 021, 022, 023, 024 — divergent-by-design at fix-time re-audit). With DUPL-018 closed via the FOLLOW-001/002 gap-closer in 2.9.33, every DUPL-NNN ID has terminal status. Burn-down complete.
...
| 18 | `add_follower` / `stop_follower` / `is_same_group` | ✅ **FIXED** (2.9.33) — closed via `FOLLOW-001` + `FOLLOW-002` in `docs/parity/ACT_COMM_C_AUDIT.md`. `mud/characters/follow.py:add_follower` now gates the master-side `"$n now follows you."` on `can_see_character(master, follower)` (ROM src/act_comm.c:1602-1603); `:stop_follower` gates both TO_VICT/TO_CHAR messages on `can_see_character(master, follower) and follower.room is not None` (ROM 1625-1629). `mud/commands/group_commands.py` copies were already ROM-faithful — that asymmetry was the bug. `is_same_group` is unchanged (already ROM-faithful in both copies). Tests: `tests/integration/test_follow_can_see_gating.py` (3 tests). | DUPL-018 |
...
CLEANUP burn-down complete as of 2.9.31; DUPL-018 closed in 2.9.33. Final tally: **3 rows ✅ FIXED via cleanup canonical** (DUPL-010, 011, 019 — genuinely mechanical, canonical existed or trivial to create), **9 rows ✅ MATCH** (DUPL-009, 012, 013, 014, 015, 016, 017, 020, 021, 022, 023, 024 — fix-time re-audit found divergent-by-design semantics; mechanical consolidation would change behavior or re-introduce known bug classes — per advisor: "expect 2-3 of the 'CLEANUP' rows to reclassify when you fix-time re-read them" — actual: 9 reclassified), **1 row refiled-then-closed** (DUPL-018 — escalated from CLEANUP to a real ROM-parity gap and closed via `FOLLOW-001` + `FOLLOW-002` in 2.9.33). Methodology lesson: the original subagent had ~30% precision on CLEANUP classifications (vs ~50% on ❌). Fix-time re-audit is mandatory for every row, no exceptions — and the refile-then-close path on DUPL-018 confirmed the methodology even for the one row that didn't fit either bucket cleanly.
