...
| ID | Site | Issue | Severity | Status | Notes |
|----|------|-------|----------|--------|-------|
| PARALLEL-010 | `mud/commands/combat.py:683-688` (`do_flee` movement branch) | After successful flee, the code does: `if room and hasattr(room, "characters") and char in room.characters: room.characters.remove(char) ... new_room.characters.append(char)`. `Room` defines `people` (see `mud/models/room.py:131`); there is **no** `characters` attribute. | MEDIUM (silent no-op) | ❌ ACTIVE-BUG | The `hasattr(room, "characters")` gate is always False, so the old-room removal silently skips. Worse, `new_room.characters.append(char)` will raise `AttributeError` at runtime on every successful flee — except this branch sits inside a broad `try/except Exception as e: messages.append(f"Flee failed: {e}")` block (line 695), so the player sees "Flee failed: ..." despite having already paid the move cost and decremented `char.move` at line 699. Net effect: flee appears to fail, the character does not actually move, and `room.people`/`new_room.people` are never updated. Use `char_from_room(char) / char_to_room(char, new_room)` (canonical movement helpers in `mud/world/char_utils.py`) — `char.room` assignment alone is insufficient because `room.people` membership is a separate INV-010 contract. Recommend filing as a single-gap closer. |
