...
| 17 | `order` | `mud/commands/group_commands.py:473` | `src/act_comm.c:1650` | 0 | 1 | 0 | 0 | 1 | ⚠️ Partial (2.9.62 probe — ORDER-001) | **BCAST-017** — Broadcast covered; secondary divergences open. `do_order` (`mud/commands/group_commands.py:519,554`) delivers the TO_VICT order via `send_to_char(order_message, occupant/victim)` — real-time correct. Two minor divergences from ROM `act_comm.c:1752-1753`: (a) Python formats `f"{char.name} orders you..."` manually rather than using `act()` `$n` substitution, bypassing visibility gating; (b) ROM checks `arg2` (second word) for the "delete"/"mob" guard while Python checks word 0. Filed as **ORDER-001** in Blocked rows. |
...
### ORDER-001 — do_order bypasses act() visibility gating + wrong word-position guard (surfaced 2026-05-27 during Class A burn-down probe)

**Status**: ❌ OPEN (low priority — broadcast itself is real-time correct). `mud/commands/group_commands.py:do_order` formats `f"{char.name} orders you to '…'"` as a raw string and delivers via `send_to_char(occupant/victim, message)`. ROM `act_comm.c:1752-1753` uses `act(buf, ch, NULL, och, TO_VICT)` which substitutes `$n` according to the recipient's perception of the actor (handles invisibility, dark rooms, etc.). Python's manual format skips the `can_see_character` gate so wiz-invis orderers reveal themselves. Adjacent: ROM checks `arg2` (second word — the command being ordered) for the "delete"/"mob" guard, Python checks `command_parts[0]` (the target name), which is a separate semantic bug.
