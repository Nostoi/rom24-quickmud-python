...
| 2 | `clone` | `mud/commands/imm_search.py:375` | `src/act_wiz.c:2296` | 2 | 1 | 0 | 0 | 2 | ✅ FIXED (2.9.62) | **BCAST-002** — Both branches now emit ROM TO_ROOM. Obj branch `$n has created $p.` (ROM `:2406`) fixed in 2.9.61; mob branch `$n has created $N.` (ROM `:2450`) fixed in 2.9.62 after CLONE-001 unblock. Tests: `tests/integration/test_clone_broadcasts.py::{test_clone_object_emits_to_room_broadcast,test_clone_mob_emits_to_room_broadcast}`. Adjacent (out of scope for BCAST): return strings `"You clone $p."` / `"You clone $N."` are unsubstituted ROM template literals — `$p`/`$N` are not expanded to the clone's short_descr. |
...
| 6 | `enter` | `mud/commands/movement.py:47` | `src/act_enter.c:43` | 5 | 0 | 0 | 0 | 5 | ✅ COVERED (2.9.62) | **BCAST-006** — FALSE POSITIVE. `do_enter` delegates to `move_character_through_portal` at `mud/world/movement.py:470`, which emits all 5 ROM acts via `broadcast_room`: departure `$n steps into $p.` (line 547), arrival `$n has arrived [through $p].` (line 575), and fade-in-place variants (lines 634, 644). Audit's static scan inspected the dispatcher, not the helper. Same helper-transitivity pattern as BCAST-019. |
...
| 9 | `group` | `mud/commands/group_commands.py:165` | `src/act_comm.c:1736` | 0 | 3 | 2 | 0 | 5 | ✅ FIXED (2.9.63 — GROUP-001) | **BCAST-009** — All ROM `act_comm.c:1842-1854` add/remove TO_VICT and TO_NOTVICT acts now reach live sockets. New `_send_to_char_sync` helper in `group_commands.py` fires `asyncio.create_task(send_to_char(...))` for connected players; `.messages` retained as test fallback. Both add and remove branches route TO_VICT via that helper and TO_NOTVICT via a `room.people` loop excluding `{char, victim}`. Regression: `tests/integration/test_group_broadcasts.py` (2/2). |
| 10 | `gtell` | `mud/commands/group_commands.py:303` | `src/act_comm.c:1948` | 0 | 1 | 0 | 0 | 1 | ✅ COVERED (2.9.62) | **BCAST-010** — FALSE POSITIVE. `do_gtell` (`mud/commands/group_commands.py:303-334`) iterates `character_registry` for each group member and calls `mud.net.protocol.send_to_char(message, gch)` (line 331) — async real-time delivery equivalent to ROM's `act_new("$n tells the group '$t'", TO_VICT, POS_SLEEPING)` per-member loop (ROM `act_comm.c:2003-2004`). Audit's static scan didn't trace into the group-member loop. |
...
| 14 | `mload` | `mud/commands/imm_load.py:52` | `src/act_wiz.c:2447` | 1 | 1 | 0 | 0 | 0 | ✅ FIXED (2.9.61) | **BCAST-014** — TO_ROOM `$n has created $N!` emitted after spawn/placement, before the wiznet log (mirrors ROM `src/act_wiz.c:2512`). Uses `victim.short_descr` → `prototype.short_descr` → `victim.name` fallback chain since spawned MobInstances populate `name` from `proto.short_descr` rather than copying `short_descr`. Test: `tests/integration/test_mload_oload_broadcasts.py::test_mload_emits_to_room_broadcast_with_victim_short_descr`. |
| 15 | `oload` | `mud/commands/imm_load.py:95` | `src/act_wiz.c:2479` | 1 | 1 | 0 | 0 | 0 | ✅ FIXED (2.9.61) | **BCAST-015** — TO_ROOM `$n has created $p!` emitted after spawn/placement, before the wiznet log (mirrors ROM `src/act_wiz.c:2566`). Same `short_descr` → `prototype.short_descr` → `name` fallback chain as BCAST-014. Test: `tests/integration/test_mload_oload_broadcasts.py::test_oload_emits_to_room_broadcast_with_obj_short_descr`. |
...
| 17 | `order` | `mud/commands/group_commands.py:473` | `src/act_comm.c:1650` | 0 | 1 | 0 | 0 | 2 | ✅ FIXED (2.9.63 — ORDER-001) | **BCAST-017** — Visibility gating landed. `do_order` now routes TO_VICT through `_pers_gated(char, occupant_or_victim)` which mirrors ROM `pers(ch, vict)` semantics: returns `"someone"` when the recipient fails `can_see_character(viewer, actor)` (wiz-invis above viewer trust, dark room, etc.), else the actor's name. Wraps the delivery in the canonical `_send_to_char_sync` helper. The pre-fix `send_to_char(order_message, victim)` call also had its args reversed and was never awaited — both fixed. Also dropped the secondary "ROM checks arg2" claim from the audit: Python's `command.split(None, 1)[0]` IS already the second word of the original input (i.e. ROM's `arg2`). Regression: `tests/integration/test_order_broadcasts.py` (2/2). |
...
| 19 | `reply` | `mud/commands/communication.py:244` | `src/act_comm.c:928` | 0 | 1 | 0 | 0 | 0 | ✅ COVERED (2.9.61 probe) | **BCAST-019** — False positive. `do_reply` (`mud/commands/communication.py:244-252`) is a five-line dispatcher that delegates to `do_tell(char, f"{target.name} {args}")`. The ROM TO_VICT `$n tells you '$t'` (`src/act_comm.c:946-947`) is already emitted by `do_tell`'s per-victim path (`communication.py:70-83`). Same helper-transitivity pattern as BCAST-001/004/005/007/008/020/026/029. No code change. |
...
| 21 | `rest` | `mud/commands/position.py:209` | `src/act_move.c:1078` | 12 | 0 | 0 | 0 | 12 | ✅ COVERED (2.9.62) | **BCAST-021** — FALSE POSITIVE. `do_rest` calls `_broadcast(ch, …)` at lines 236, 245, 254, 263, 269, 278 covering all position × furniture branches; `_broadcast` (`mud/commands/position.py:91`) wraps `broadcast_room(room, msg, exclude=ch)`. All 12 ROM `act_move.c:1110-1246` non-TO_CHAR acts present. Audit's static scan didn't recognize the local `_broadcast` helper. |
| 22 | `sit` | `mud/commands/position.py:290` | `src/act_move.c:1217` | 10 | 0 | 0 | 0 | 10 | ✅ COVERED (2.9.62) | **BCAST-022** — FALSE POSITIVE. `do_sit` calls `_broadcast(ch, …)` at lines 316, 326, 336, 341, 350, 359 for all furniture × position branches (including ROM bug-faithful `$n wakes and sits at $p` on SIT_ON). Same `_broadcast` → `broadcast_room` helper as BCAST-021. |
| 23 | `sleep` | `mud/commands/position.py:371` | `src/act_move.c:1343` | 4 | 0 | 0 | 0 | 4 | ✅ COVERED (2.9.62) | **BCAST-023** — FALSE POSITIVE. `do_sleep` calls `_broadcast(ch, "$n goes to sleep.")` (line 383) for no-furniture and `_broadcast(ch, tr, obj)` (line 414) for all furniture branches — mirrors ROM `act_move.c:1375-1449`. Same `_broadcast` → `broadcast_room` helper as BCAST-021. |
| 24 | `stand` | `mud/commands/position.py:111` | `src/act_move.c:967` | 8 | 0 | 0 | 0 | 8 | ✅ COVERED (2.9.62) | **BCAST-024** — FALSE POSITIVE. `do_stand` calls `_broadcast` at lines 145, 159, 177, 191 — all SLEEPING/RESTING/SITTING × no-furniture/AT/ON/IN branches (including ROM bug-faithful "stands on $p" in SIT_IN). Same `_broadcast` → `broadcast_room` helper as BCAST-021. |
...
| 28 | `value` | `mud/commands/shop.py:1018` | `src/act_obj.c:2904` | 0 | 4 | 0 | 0 | 4 | ✅ COVERED (2.9.62) | **BCAST-028** — FALSE POSITIVE. ROM `act_obj.c:2904-3018` `do_value` has only TO_VICT keeper→customer acts (no TO_ROOM). Python `do_value` delivers all responses via `_keeper_says()` returned to the customer (the TO_VICT-equivalent contract). No TO_ROOM exists in ROM to match against. |
...
| 30 | `bash` | `mud/skills/handlers.py:1321` | `src/fight.c:2455-2484` | 0 | 2 | 2 | 0 | 4 | ✅ FIXED (2.9.62) | **BCAST-030** — FIXED 2.9.62: `bash` handler now emits all 4 ROM acts (success TO_VICT `$n sends you sprawling…`, success TO_NOTVICT `$n sends $N sprawling…`, failure TO_NOTVICT `$n falls flat on $s face.`, failure TO_VICT `You evade $n's bash, causing $m to fall flat on $s face.`). Added `_objective_pronoun` helper for `$m` substitution and `_to_vict_send` / `_notvict_broadcast` delivery helpers (two-actor exclude for TO_NOTVICT). Regression: `tests/integration/test_bash_broadcasts.py` (2/2). |
| 31 | `buy` | `mud/commands/shop.py:719` | `src/act_obj.c:2470` | 3 | 6 | 0 | 0 | 3 | ✅ COVERED (2.9.62) | **BCAST-031** — FALSE POSITIVE. `do_buy` (`mud/commands/shop.py:831-837`) calls `room.broadcast(f"{buyer_name} buys {item_name}[{quantity}].", exclude=char)` matching ROM `act_obj.c:2734-2745` TO_ROOM `$n buys $p[N].` Pet-shop branch (line 631) uses `current_room.broadcast(…)`. Audit's static scan counted distinct ROM acts (9) without recognizing that several are TO_VICT keeper→customer messages delivered as returned strings in Python's design. |
| 32 | `force` | `mud/commands/imm_commands.py:293` | `src/act_wiz.c:4111` | 0 | 4 | 0 | 0 | 4 | ✅ COVERED (2.9.62) | **BCAST-032** — FALSE POSITIVE. All 4 ROM `act_wiz.c:4205,4228,4251,4274,4316` TO_VICT `$n forces you to '…'` acts are delivered via `_send_to_char(vch/victim, f"{char.name} forces you to '…'")` at `imm_commands.py:325, 340, 356, 385` across all 4 force branches. No TO_ROOM acts in ROM `do_force`. (Minor adjacent: Python bypasses `act()` visibility gating — same shape as ORDER-001, lower stakes for immortal commands.) |
| 33 | `give` | `mud/commands/give.py:21` | `src/act_obj.c:634` | 0 | 4 | 2 | 0 | 6 | ✅ COVERED (2.9.62) | **BCAST-033** — FALSE POSITIVE. `do_give` (`mud/commands/give.py:83-94, 139-150`) emits TO_NOTVICT via `_broadcast_to_room_observers`, TO_VICT via `victim.messages.append` + `send_to_char` task (line 238), and TO_CHAR via return string — for both item-give and money-give paths. Matches ROM `act_obj.c:832-836`. (INV-013 single-delivery covers this surface.) |
| 34 | `pick` | `mud/commands/doors.py:529` | `src/act_move.c:809` | 3 | 3 | 0 | 0 | 1 | ✅ FIXED (2.9.62) | **BCAST-034** — All 3 TO_ROOM emits added: portal `$n picks the lock on $p.` (ROM `:907`), container `$n picks the lock on $p.` (ROM `:945`), door `$n picks the $d.` (ROM `:981`, with `$d` resolved to first word of `pexit->keyword` via `_door_keyword`). Regression: `tests/integration/test_pick_broadcasts.py` (2/2 — container + door; portal path shares container's code shape). |
| 35 | `purge` | `mud/commands/imm_load.py:183` | `src/act_wiz.c:2532` | 1 | 3 | 1 | 0 | 2 | ✅ FIXED (2.9.62) | **BCAST-035** — All 3 emits added: room-purge TO_ROOM `$n purges the room!` (ROM `:2605`) via `broadcast_room`; PC-disintegrate TO_NOTVICT `$n disintegrates $N.` (ROM `:2633`) and NPC-purge TO_NOTVICT `$n purges $N.` (ROM `:2645`) via new local `_notvict_broadcast` helper (mirrors `broadcast_room` but supports a two-actor exclude set so the about-to-be-extracted victim is filtered out). Regression: `tests/integration/test_purge_broadcasts.py` (2/2). |
| 36 | `recall` | `mud/commands/session.py:328` | `src/act_move.c:1529` | 3 | 0 | 0 | 0 | 3 | ✅ COVERED (2.9.62) | **BCAST-036** — FALSE POSITIVE. `do_recall` (`mud/commands/session.py:355-411`) emits all 3 ROM acts via `room.broadcast(…, exclude=ch)`: `prays for transportation!` (line 356, ROM departure), `disappears.` (line 399, ROM old-room exit), `appears in the room.` (line 411, ROM arrival in target room). |
| 37 | `sell` | `mud/commands/shop.py:873` | `src/act_obj.c:2810` | 1 | 4 | 0 | 0 | 5 | ✅ COVERED (2.9.62) | **BCAST-037** — FALSE POSITIVE. `do_sell` (`mud/commands/shop.py:941-945`) calls `room.broadcast(f"{seller_name} sells {item_name}.", exclude=char)` matching ROM `act_obj.c:2923` TO_ROOM `$n sells $p.` The 4 TO_VICT keeper→customer acts are delivered as returned strings (same design as BCAST-028/031). |
| 38 | `steal` | `mud/commands/thief_skills.py:78` | `src/act_obj.c:2126` | 0 | 1 | 1 | 0 | 2 | ✅ FIXED (2.9.63 — STEAL-001) | **BCAST-038** — All ROM `act_obj.c:2222-2223` acts now reach live sockets. New `_send_to_char_sync` helper in `thief_skills.py` fires `asyncio.create_task(send_to_char(...))` for connected players and falls back to `.messages` for tests. `_steal_failure` routes TO_VICT via that helper and TO_NOTVICT via a `room.people` loop excluding `{char, victim}`. Regression: `tests/integration/test_steal_broadcasts.py` (1/1). |
| 39 | `transfer` | `mud/commands/imm_commands.py:218` | `src/act_wiz.c:772` | 2 | 1 | 0 | 0 | 3 | ✅ COVERED (2.9.62) | **BCAST-039** — FALSE POSITIVE. `do_transfer` (`mud/commands/imm_commands.py:273-283`) emits all 3 ROM acts: `_act_room(old_room, victim, "$n disappears in a mushroom cloud.")` (ROM `act_wiz.c:870`), `_act_room(location, victim, "$n arrives from a puff of smoke.")` (ROM `:871`), `_send_to_char(victim, f"{char_name} has transferred you.")` (ROM `:875`). `_act_room` iterates `room.people` and delivers via `_send_to_char` (buffered, real-time). |
...
### GROUP-001 — do_group TO_NOTVICT delivered via `.messages` only (surfaced 2026-05-27 during Class A burn-down probe, ✅ FIXED 2026-05-27 in 2.9.63)

**Status**: ✅ FIXED in 2.9.63. New `_send_to_char_sync` helper in `mud/commands/group_commands.py` fires `asyncio.create_task(send_to_char(...))` for connected players. Both add (`act_comm.c:1850-1854`) and remove (`act_comm.c:1842-1846`) branches route TO_VICT and TO_NOTVICT through it. Regression: `tests/integration/test_group_broadcasts.py` (2/2). Out of scope: `$N isn't following you.` rendering via `act_new()` — minor, not pursued.

**Historical Status**: ❌ OPEN. `mud/commands/group_commands.py:do_group` emits the ROM TO_VICT (`$N joins $n's group.`) and TO_NOTVICT (`$N follows $n.`) acts via `victim.messages.append` and an `occupant.messages.append` loop respectively — but never through `send_to_char` or `broadcast_room`. The `.messages` list is the test fallback per AGENTS.md "Message Delivery (Architectural Divergence)" — players connected to real sockets see nothing on group-add/remove. Also `$N isn't following you.` is returned as a plain string where ROM uses `act_new(..., TO_CHAR, POS_SLEEPING)` which goes through the visibility-gated delivery path.

**Fix shape**: replace the `.messages` loops with `broadcast_room(room, msg, exclude={ch, victim})` (or a two-actor exclude helper like `_notvict_broadcast` from imm_load.py / handlers.py) plus an explicit `send_to_char` for the TO_VICT path. Add an integration test in `tests/integration/test_group_broadcasts.py` asserting both the connected-bystander and connected-victim receive the message.

**Blocks**: BCAST-009 row closure.



### ORDER-001 — do_order bypasses act() visibility gating + wrong word-position guard (surfaced 2026-05-27 during Class A burn-down probe, ✅ FIXED 2026-05-27 in 2.9.63)

**Status**: ✅ FIXED in 2.9.63. New `_pers_gated(actor, viewer)` helper mirrors ROM `src/handler.c:pers` — returns `"someone"` when `can_see_character(viewer, actor)` fails, else actor's name. Both all-targets and single-target branches build `order_message` via `_pers_gated(char, occupant_or_victim)` and deliver through `_send_to_char_sync`. Pre-fix the raw `send_to_char(order_message, victim)` call had args reversed and was never awaited; fixed in same pass. Audit note (b) — "ROM checks arg2, Python checks word 0" — was incorrect: Python's `command.split(None, 1)[0]` IS the second word of the original input (ROM's `arg2`). No fix needed there. Regression: `tests/integration/test_order_broadcasts.py` (2/2).

**Historical Status**: ❌ OPEN.

**Fix shape**: route the order message through the same `act`-equivalent path used by other immortal commands, and fix the word-position check to compare against the second arg. Add a test pinning that a wiz-invis orderer is rendered as "someone" to a low-trust recipient.

**Blocks**: BCAST-017 row closure (broadcast delivery is correct; row stays ⚠️ Partial until visibility gating lands).



### STEAL-001 — _steal_failure TO_VICT and TO_NOTVICT delivered via `.messages` only (surfaced 2026-05-27 during Class A burn-down probe, ✅ FIXED 2026-05-27 in 2.9.63)

**Status**: ✅ FIXED in 2.9.63. New `_send_to_char_sync` helper in `mud/commands/thief_skills.py` fires `asyncio.create_task(send_to_char(...))` for connected players and falls back to `.messages` for tests. `_steal_failure` routes TO_VICT via that helper and TO_NOTVICT via a `room.people` loop excluding `{char, victim}`. Regression: `tests/integration/test_steal_broadcasts.py` (1/1).

**Historical Status**: ❌ OPEN. `mud/commands/thief_skills.py:_steal_failure` (lines 168-233) handles ROM `act_obj.c:2222-2223` TO_VICT (`$n tried to steal from you.`) and TO_NOTVICT (`$n tried to steal from $N.`) by appending to `victim.messages` (line 187) and looping over `room.people` appending to `bystander.messages` (line 192). No `send_to_char` or `broadcast_room` calls — connected players see nothing when someone fails a steal on them. Success item-path (`_steal_item`) is correct: ROM `act_obj.c:2322-2326` is TO_CHAR-only and Python returns the right string.

**Fix shape**: replace the `.messages` loops with `send_to_char` for the victim TO_VICT and `broadcast_room(room, msg, exclude={ch, victim})` (or a two-actor helper) for the TO_NOTVICT. Add an integration test in `tests/integration/test_steal_broadcasts.py` asserting both connected-victim and connected-bystander receive the message on a failed steal.

**Blocks**: BCAST-038 row closure.



### CLONE-001 — do_clone mob branch imports non-existent LEVEL_ constants (surfaced 2026-05-27 during BCAST-002 obj-branch closure, ✅ FIXED 2026-05-27 in 2.9.62)

**Status**: ✅ FIXED 2026-05-27 (2.9.62) — Option 2 path taken. Added `LEVEL_AVATAR = 52`, `LEVEL_DEMI = 54`, `LEVEL_IMMORTAL_TIER = 55`, `LEVEL_GOD = 56` in `mud/models/constants.py` (existing `LEVEL_IMMORTAL = 52` threshold left untouched to avoid wide blast radius). Updated `mud/commands/imm_search.py:do_clone` to use `LEVEL_IMMORTAL_TIER` for the `mob_level > 10` gate (ROM `IMMORTAL = 55`, not the threshold), use `LEVEL_ANGEL` for `mob_level > 0` (matching ROM), and add the unconditional `IS_TRUSTED(AVATAR)` floor that was missing. Regression: `tests/integration/test_act_wiz_command_parity.py::test_clone_mob_success_no_import_error_and_places_clone`. **Unblocks BCAST-002 mob branch**.
...
**Status**: ✅ FIXED. `mud/commands/imm_load.py` now reads `mob_registry`/`obj_registry` and uses `spawn_object`; `mud/commands/imm_search.py` `do_clone` object branch likewise uses `spawn_object` and skips the read-only `name` property in its attribute-copy loop (a fourth layered bug surfaced once the ImportError was lifted — folded into the same commit). Three success-path regression tests under `tests/integration/test_act_wiz_command_parity.py::test_{mload,oload,clone_object}_success_*` pin the fix. BCAST-002 / 014 / 015 are unblocked.

#### Pre-fix detail (kept for the audit trail)
