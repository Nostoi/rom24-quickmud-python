...
| 38 | `steal` | `mud/commands/thief_skills.py:78` | `src/act_obj.c:2126` | 0 | 1 | 1 | 0 | 1 | ⚠️ Partial (2.9.62 probe — STEAL-001) | **BCAST-038** — Partial. Success item-path matches ROM (ROM `act_obj.c:2322-2326` is TO_CHAR-only, Python returns the correct string). However the failure path `_steal_failure` (`mud/commands/thief_skills.py:168-233`) delivers ROM's TO_VICT (`$n tried to steal from you.`) and TO_NOTVICT (`$n tried to steal from $N.`) via `.messages` list appends only (lines 187, 192) — no `send_to_char` / `broadcast_room` calls, so live-socket players see nothing. Filed as **STEAL-001** in Blocked rows. |
