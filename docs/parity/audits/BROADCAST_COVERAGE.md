...
| 15 | `oload` | `mud/commands/imm_load.py:95` | `src/act_wiz.c:2479` | 1 | 0 | 0 | 0 | 0 | ❌ | **BCAST-015** — ROM `act("$n has created $p!", ch, obj, NULL, TO_ROOM)` at `src/act_wiz.c:2566`. WIZLOAD-001 unblocked the success path; standard `rom-gap-closer` adds the TO_ROOM emit after the spawn/placement. |
