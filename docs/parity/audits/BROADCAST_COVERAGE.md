...
| 9 | `group` | `mud/commands/group_commands.py:165` | `src/act_comm.c:1736` | 0 | 3 | 2 | 0 | 0 | ⚠️ Partial (2.9.62 probe — GROUP-001) | **BCAST-009** — Partial. `do_group` (`mud/commands/group_commands.py:265-300`) does emit TO_VICT via `victim.messages` and TO_NOTVICT via room loop, but uses the test-only `.messages` list rather than `send_to_char` / `broadcast_room` — bystanders connected via real sockets see nothing. Also `$N isn't following you.` is returned as a plain string where ROM uses `act_new(..., TO_CHAR, POS_SLEEPING)`. Filed as **GROUP-001** in Blocked rows. |
...
### GROUP-001 — do_group TO_NOTVICT delivered via `.messages` only (surfaced 2026-05-27 during Class A burn-down probe)

**Status**: ❌ OPEN. `mud/commands/group_commands.py:do_group` emits the ROM TO_VICT (`$N joins $n's group.`) and TO_NOTVICT (`$N follows $n.`) acts via `victim.messages.append` and an `occupant.messages.append` loop respectively — but never through `send_to_char` or `broadcast_room`. The `.messages` list is the test fallback per AGENTS.md "Message Delivery (Architectural Divergence)" — players connected to real sockets see nothing on group-add/remove. Also `$N isn't following you.` is returned as a plain string where ROM uses `act_new(..., TO_CHAR, POS_SLEEPING)` which goes through the visibility-gated delivery path.
