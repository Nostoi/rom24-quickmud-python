...
| OLC_SAVE-009 | IMPORTANT | src/olc_save.c:826-872 | — | No Python help-save path. ROM `save_helps`/`save_other_helps` not ported. | ✅ FIXED — area-grouped JSON shape: `_serialize_help` (`mud/olc/save.py`) emits `{level, keywords, text}`; `save_area_to_json` includes a per-area `helps` list; loader-side `_load_helps_from_json` (`mud/loaders/json_loader.py`) rehydrates `area.helps` and registers each entry with `register_help`. HAD/`save_other_helps` standalone-help-file fan-out remains N/A under JSON-authoritative framing (Python has no global `had_list`; helps live on their owning area). Locked by `tests/integration/test_olc_save_009_area_helps_round_trip.py` (3 cases: section emit, full save→load round-trip with help_registry rehydration, empty-helps no-crash). |
