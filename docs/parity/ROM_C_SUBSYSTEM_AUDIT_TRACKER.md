...
| **Core Game Loop** | | | | | |
| `update.c` | P0 | ✅ Audited | `mud/game_loop.py`, `mud/advancement.py` | 100% | May 30, 2026 — `GL-025` closed: `char_update` now mirrors ROM order for PC worn-light decay, idle timer, and condition decay before affect expiry and plague/poison/incap/mortal damage (`update.c:721-862`). May 27 — `ARITH-203/204` closed: plague-tick mana/move drain floors removed (raw subtraction per `update.c:843-845`). `GL-024` closed: plague tick's `if (af->level == 1) continue;` skip (`update.c:818-819`) now mirrored — level-1 plague is dormant (drain + `damage()` moved under the `af_level > 1` guard). May 17 — `GL-010`/`GL-017` closed (affect-tick same-type wear-off handling); `GL-021` (point-pulse `wiznet("TICK!")` ordering); `GL-022` (level-up banner/log order); `GL-023` (combat→XP verification). |
| `handler.c` | P1 | ✅ **COMPLETE!** | `mud/handler.py`, `mud/world/`, `mud/models/` | **100%** | 🎉🎉🎉 **FULL PARITY ACHIEVED - ALL 75 FUNCTIONS IMPLEMENTED!** 🎉🎉🎉 Jan 4 (74 functions) + 2026-06-08: `affect_join` merge semantics added (was partial) — affects system now 100% (11/11). See HANDLER_C_AUDIT.md |
