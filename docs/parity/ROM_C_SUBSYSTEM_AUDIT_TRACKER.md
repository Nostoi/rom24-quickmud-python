...
| **Utilities & Helpers** | | | | | |
| `const.c` | P3 | ⚠️ Partial | `mud/models/constants.py`, `mud/models/races.py`, `mud/models/classes.py`, `mud/skills/groups.py`, `mud/wiznet.py`, `mud/world/movement.py`, `mud/commands/equipment.py` | 80% | Apr 29, 2026 — `CONST_C_AUDIT.md` Phase 1–3 complete. 13/16 tables ✅ AUDITED (item, wiznet, attack, race, pc_race, class, int_app, liq, skill, group + str_app.carry/wield columns). 7 gaps filed. **CRITICAL** (open): CONST-002 `GET_HITROLL` missing `str_app[STR].tohit`, CONST-003 `GET_DAMROLL` missing `str_app[STR].todam`, CONST-004 `GET_AC` missing `dex_app[DEX].defensive`, CONST-005 `advance_level` missing `con_app[CON].hitp` + `number_range(hp_min,hp_max)`. **IMPORTANT** (open): CONST-001 `title_table` (480 entries) — defer to NANNY-009 dedicated session; CONST-006 `advance_level` missing `wis_app[WIS].practice`. **MINOR**: CONST-007 `weapon_table` — defer to OLC audit (BIT-style). Tracker holds at ⚠️ Partial pending CONST-002..006 closures via `/rom-gap-closer`. See `docs/parity/CONST_C_AUDIT.md`. |
...
| `bit.c` | P3 | ✅ AUDITED | `mud/utils/prefix_lookup.py`, `mud/commands/remaining_rom.py` | 90% | Apr 29, 2026 — `flag_lookup` adjacent helper (lookup.c) ported as `prefix_lookup_intflag` (TABLES-002); `do_flag` inline accumulator faithfully mirrors ROM `do_flag` (not ROM `flag_value`). 3 MINOR gaps deferred to OLC audit (BIT-001 standalone `flag_value`, BIT-002 `flag_string` decoder, BIT-003 `flag_stat_table`+`is_stat`) — no current Python consumer; close alongside OLC port. See BIT_C_AUDIT.md |
