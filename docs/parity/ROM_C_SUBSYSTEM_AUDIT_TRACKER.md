...
| **Core Game Loop** | | | | | |
| `update.c` | P0 | ✅ Audited | `mud/game_loop.py`, `mud/advancement.py` | 100% | May 27, 2026 — `ARITH-203/204` closed: plague-tick mana/move drain floors removed (raw subtraction per `update.c:843-845`). **Open follow-up: `GL-024`** — plague tick's `if (af->level == 1) continue;` skip (`update.c:818-819`) is not mirrored; Python still drains + damages at level-1 plague. May 17 — `GL-010`/`GL-017` closed (affect-tick same-type wear-off handling); `GL-021` (point-pulse `wiznet("TICK!")` ordering); `GL-022` (level-up banner/log order); `GL-023` (combat→XP verification). |
