...
| GAIN-005 | MINOR | `src/skills.c:70,137-143,181-244` (`act("$N ...")` → `PERS(mob)` → `mob->short_descr`) | `mud/commands/remaining_rom.py:189` (`_gain_trainer_name`) | **Every trainer line printed the placeholder "The trainer" instead of the trainer's name.** `_gain_trainer_name` (the GAIN-004 act-cap helper) read `trainer.short_descr or "The trainer"`, but a real spawned `MobInstance` leaves `.short_descr` **None** and stores the C `short_descr` in `.name` (`mud/spawning/templates.py:447` `name=proto.short_descr or proto.player_name`). So at every live ACT_GAIN trainer (e.g. newthalos guildmaster 9500, short_descr "the guildmaster") the no-arg/convert/points/group/skill lines rendered "The trainer says/tells you ..." rather than ROM's "The guildmaster ...". The GAIN-004 tests missed it because they set `short_descr` explicitly on a `Character`. Surfaced by the `gain_convert_points` differential scenario (FINDING-039). | ✅ FIXED (2.14.204) — use the established `short_descr or name` idiom (cf. `make_corpse`): `getattr(trainer, "short_descr", None) or getattr(trainer, "name", None) or "someone"`, then `capitalize_act_line`. Test: `tests/integration/test_do_gain_act_gain_bit.py::test_gain_trainer_name_falls_back_to_name_when_short_descr_unset` (mob with `short_descr=None`, `name="the guildmaster"` → "The guildmaster says 'Pardon me?'") + the `gain_convert_points` differential replay. |
