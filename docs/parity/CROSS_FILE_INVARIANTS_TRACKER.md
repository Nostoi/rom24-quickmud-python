...
| INV-025 | MOBPROG-ACT-TRIGGER-DISPATCH | `src/comm.c:2384-2385` — inside `act()` itself, after the per-recipient buffer is formatted, every NPC `to` (`to->desc == NULL` or `to->desc->connected != CON_PLAYING`) receives `mp_act_trigger(buf, to, ch, arg1, arg2, TRIG_ACT)` provided the global `bool MOBtrigger` (`src/comm.c:311`) is TRUE. ROM toggles `MOBtrigger = FALSE` around free-form / recursive `act()` paths (`src/act_comm.c:1090-1093 do_emote`, `src/act_comm.c:1135/1187 do_pmote`, `src/act_obj.c:832-836 do_give`, `src/mob_cmds.c:333-335 do_mpasound`) so those broadcasts do not fire a TRIG_ACT — emote/pmote specifically so a player cannot forge an arbitrary trigger phrase via free-form text. Two-level contract: (i) every act()-driven broadcast must dispatch TRIG_ACT to NPC recipients in the room; (ii) the MOBtrigger recursion guard must suppress that dispatch when callers explicitly opt out. | (a) `mud/mobprog.py` exposes a module-level `MOBtrigger: bool = True` flag and a `disable_mobtrigger()` context manager (lines 19-46) — the Python port of ROM's `MOBtrigger = FALSE; act(...); MOBtrigger = TRUE;` pattern. Nesting is safe (previous value is restored, not unconditionally set TRUE) so future recursive paths can stack the guard without leaking. (b) `mud/mobprog.py:mp_act_trigger_room(message, room, ch, *, arg1, arg2, exclude)` (the per-room dispatcher) iterates `room.people`, skips PCs / `ch` / `exclude`, fires `mp_act_trigger` per NPC, and short-circuits on `MOBtrigger == False`. (c) **EMOTE-003 correction (2.12.5): `do_emote` is a TRIG_ACT *suppressor*, not a producer.** ROM `src/act_comm.c:1090-1093` wraps both `act("$n $T", …)` calls in `MOBtrigger = FALSE; … ; MOBtrigger = TRUE;`, and `src/comm.c:2384` dispatches only `else if (MOBtrigger)` — so an emote never fires a TRIG_ACT (the guard against players forging trigger phrases via free-form text). The 2.9.40 enforcement mis-identified `do_emote` as the canonical producer and made `mud/commands/communication.py:do_emote` call `mp_act_trigger_room(args, char.room, char)` at runtime — a shipped behavioral bug (`emote bows` tripping an NPC scripted on "bows"). The call was **removed**; the per-listener message fan-out (PCs still see the emote) is unchanged. The contract's positive (producer) leg is now anchored at `do_stand` — a genuine ROM TRIG_ACT producer (`act("$n stands up.", …)`, no MOBtrigger wrap) — and the object/door command sites (e), (f); the suppression leg is locked by the emote no-fire + `disable_mobtrigger()`-gate tests. (d) `mud/game_loop.py:_act_to_room` handles update-loop act() sites that require per-recipient PERS formatting before trigger dispatch; plague tick room messages (`src/update.c:803-804`, `:836-837`) now exclude the actor, mask unseen actor names, and call `mp_act_trigger` for NPC recipients. (e) `mud/commands/doors.py:_broadcast_act_to_room` handles `do_open` actor-room act() lines (`src/act_move.c:384`, `:412`, `:436`) by broadcasting once and dispatching `TRIG_ACT` to NPC recipients. (f) `mud/mobprog.py:mp_reverse_act_trigger_room` handles the `do_open`/`do_close` **reverse-side** linked-room loops (`src/act_move.c:447-448`, `:545-547`) where ROM's `act("The $d opens.", rch, NULL, ..., TO_CHAR)` makes each NPC its own actor (`to==ch==rch`); the dispatcher threads the recipient itself as the trigger actor, with no `ch` to exclude. (g) `mud/commands/consumption.py:do_eat`/`do_drink` now dispatch TRIG_ACT for the eat/drink room act lines and their poisoned choke act lines (`src/act_obj.c:1238-1241,1263,1317,1342`). Pre-INV-025 follow-up, every TRIG_ACT mobprog responding to PC emotes, plague tick room lines, open-door room lines, or consumable room lines silently no-opped because Python only routed speech to mobprog or sent a baked room string. **Follow-up wiring sweep** (deferred): remaining act()-style broadcast sites (`broadcast_room`, `_push_message`, magic-item/liquid/thief/immortal command room narrations) should also fan into `mp_act_trigger_room` or a per-recipient formatter when they correspond to a ROM `act()` call. The `do_open`/`do_close` reverse-side linked-room broadcasts are now wired (2.12.4) via `mp_reverse_act_trigger_room`. The contract's producer leg is locked at the `do_stand` callsite and the suppression leg at the emote no-fire test (EMOTE-003); the sweep extends coverage but cannot regress the contract. Touched by: `mud/mobprog.py:MOBtrigger`, `mud/mobprog.py:disable_mobtrigger`, `mud/mobprog.py:mp_act_trigger_room`, `mud/commands/communication.py:do_emote` (TRIG_ACT dispatch **removed** — EMOTE-003), `mud/game_loop.py:_act_to_room`/`_char_update_tick_effects`, `mud/commands/doors.py:_broadcast_act_to_room`/`do_open`/`do_close`/`do_lock`/`do_unlock`/`do_pick`, `mud/mobprog.py:mp_reverse_act_trigger_room`, `mud/commands/consumption.py:do_eat`/`do_drink`. | `tests/integration/test_inv025_mobprog_act_trigger_dispatch.py` + `tests/integration/test_inv025_plague_tick_act_trigger_dispatch.py` + `tests/integration/test_inv025_do_open_act_trigger_dispatch.py` + `tests/integration/test_inv025_door_commands_act_trigger_dispatch.py` + `tests/integration/test_inv025_reverse_door_act_trigger_dispatch.py` + `tests/integration/test_inv025_consumption_act_trigger_dispatch.py` | ✅ ENFORCED |
...
- ~~INV-025 candidate (MOBPROG-ACT-TRIGGER-DISPATCH)~~ **enforced 2.9.40**.
  ...
  **Open (uniform, do not introduce asymmetrically):** the *reverse-side*
  linked-room broadcasts in `do_open` (`doors.py:209` "The $d opens.") and
  `do_close` (`doors.py:302` "The $d closes.") are still plain
  `broadcast_room` — they correspond to ROM's `act(..., rch, TO_CHAR)` loop
  in the linked room (`src/act_move.c:447-448`/`:545-547`), which
  `src/comm.c:2384` would dispatch TRIG_ACT for if an NPC stands in the far
  room. Left untouched in the 2.12.3 family commit to stay symmetric with the
  `do_open` precedent. If closed, close `do_open` + `do_close` together in one
  follow-up (lock/unlock/pick have no reverse-side broadcast — ROM silently
  sets bits).
