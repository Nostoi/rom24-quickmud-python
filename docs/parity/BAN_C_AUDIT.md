...
| `BAN-002` | IMPORTANT | `src/ban.c:166-168` | `mud/commands/admin_commands.py:_render_ban_listing` | Type-text fallback prints `"all"` when none of NEWBIES/PERMIT/ALL bits is set; ROM prints the empty string. | 🔄 OPEN |
| `BAN-003` | IMPORTANT | `src/ban.c:180-191` | `mud/commands/admin_commands.py:_apply_ban` | Type-token abbreviation: ROM accepts prefixes of `all`/`newbies`/`permit` (so `a`, `n`, `p`); Python uses `startswith` so it rejects them. | 🔄 OPEN |
