...
| `WIZ-052` | MEDIUM | `src/act_wiz.c:1637-1641` (`do_mstat` Thirst/Hunger/Full/Drunk line ← `condition[COND_THIRST]`, `condition[COND_HUNGER]`, `condition[COND_FULL]`, `condition[COND_DRUNK]`) | `mud/commands/imm_search.py:1126-1132` (`do_mstat`) | `do_mstat`'s condition line read the `pcdata.condition` array by **display order** (`condition[0]`→thirst, `[1]`→hunger, `[2]`→full, `[3]`→drunk) instead of by the `COND_*` enum index. The array is enum-keyed (`DRUNK=0, FULL=1, THIRST=2, HUNGER=3`, `mud/models/constants.py` `Condition`), so every label showed a different condition's value — Thirst printed the DRUNK slot, Hunger the FULL slot, Full the THIRST slot, Drunk the HUNGER slot. Surfaced 2026-06-13 by a post-EAT-006 sibling sweep for other sites mis-indexing `condition[]`. | ✅ **FIXED** (2026-06-13, 2.14.33) — reads each label from its enum slot: `thirst=condition[Condition.THIRST]`, `hunger=condition[Condition.HUNGER]`, `full=condition[Condition.FULL]`, `drunk=condition[Condition.DRUNK]` (`mud/commands/imm_search.py`). `tests/integration/test_act_wiz_command_parity.py::test_mstat_condition_line_maps_cond_indices_correctly`. |
