...
| `TRAIN-004` | MINOR | src/act_move.c:1709-1745 (`get_max_train` per-stat cap) + src/handler.c `get_max_train` | mud/commands/advancement.py:~310,~381 | `do_train` caps trainable stats at a hardcoded `max_stat = 22` (both the listing branch and the per-stat "already at maximum" check). ROM `get_max_train(ch, stat)` is **race/class dependent** — `pc_race_table[race].max_stat[stat]` plus a +4 prime-class bonus, capped at 25 (`src/handler.c`). So e.g. a human's max differs from an elf's, and the prime stat for the class gets a higher ceiling. Python over/under-caps for non-default races. Surfaced while closing TRAIN-002. Fix: port `get_max_train` from `src/handler.c` and use it in both sites instead of the literal 22. | 🔄 OPEN |
