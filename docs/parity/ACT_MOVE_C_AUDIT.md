...
| `MOVE-006` | MINOR | src/act_move.c:64-135 (`move_char` — full body) | mud/world/movement.py:359-362 (`move_character` encumbrance gate) | **Non-ROM movement gate (filed out-of-scope while closing MOVE-005; not yet fixed).** QuickMUD blocks movement with `"You are too encumbered to move."` when `get_carry_weight(char) > can_carry_w(char) or char.carry_number > can_carry_n(char)`. ROM `move_char` has **no** carry-weight/carry-number movement gate anywhere in its body — movement is gated only on **move points** (`if (ch->move < move) "You are too exhausted."`, src/act_move.c:122-126), terrain, boats, and flags. The encumbrance early-return is a QuickMUD invention with no ROM counterpart. Needs verification of whether any ROM variant/merc derivative added it; if not, it should be removed for parity. Surfaced 2026-06-02 while reading `move_char` for MOVE-005. | ❌ OPEN |
