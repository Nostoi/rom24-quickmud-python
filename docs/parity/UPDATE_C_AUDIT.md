...
| GL-027 | BUG (RNG parity) | update.c:762-768 | Same per-affect roll contract as GL-026, in the **dict-only fallback** of `tick_spell_effects` (`mud/affects/engine.py:132-146`): it decrements `spell_effects` durations but calls `number_range` **zero times** and never fades level. Reachable for **`MobInstance`** NPCs — `MobInstance.apply_spell_effect` (`mud/spawning/templates.py:546-573`) writes `spell_effects` but, unlike `Character.apply_spell_effect`, does **not** mirror into a `.affected` list, so a mob with an applied spell effect (e.g. a player casting bless/curse on it) has empty `affected` → routes through the fallback. `char_update` ticks the whole `character_registry` (mobs included), so ROM rolls one `number_range(0,4)` per `duration>0` mob affect while Python rolls none — a total desync, strictly worse than GL-026. Fix options: (a) reach RNG parity by rolling per `duration>0` effect in the fallback, or (b) make `MobInstance.apply_spell_effect` mirror into `affected` (collapsing onto the main GL-026 path). Surfaced 2026-05-31 alongside GL-026. | ⚠️ OPEN |
