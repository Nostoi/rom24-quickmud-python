...
| **BUY-009** | `do_buy()` | 2727 | `do_buy` buy-haggle discount base reads `proto.cost` (`base_cost = proto.cost`, `shop.py:826-830`); ROM computes `cost -= obj->cost / 2 * roll / 100` using the RUNTIME `obj->cost`. Diverges when `obj.cost != proto.cost` (e.g. re-buying a haggle-clamped item another player resold). | 🔄 **OPEN** — sibling of GETCOST-001; close with a failing test first. |
