...
| WEAR-011 | 1670-1677 (`wear_obj` HOLD branch) | equipment.py:451-454 (`do_hold` rejects when slot occupied) | ROM `do_hold` (= `do_wear`) HOLD branch calls `remove_obj(ch, WEAR_HOLD, fReplace=TRUE)` which auto-unequips the existing held item. Python `do_hold` instead rejects with `"You're already holding {name}."` Note `do_wear`'s HOLD branch (equipment.py:166-213) already auto-replaces correctly; only the standalone `do_hold` diverges. Required precondition for INTERP-013 collapse. | 🔄 OPEN |
