...
| **SELL-006** | `do_sell()` | 2930 | The sell-haggle bonus `cost += obj->cost / 2 * roll / 100` (`shop.py:954`) reads `proto.cost` for its base; ROM uses the RUNTIME `obj->cost`. Sell-side mirror of BUY-009 / GETCOST-001; diverges when `obj.cost != proto.cost`. | 🔄 **OPEN** — close with a failing test first (the cap at :2931 can mask it, so pick `profit_buy` high enough that the cap doesn't bind). |
