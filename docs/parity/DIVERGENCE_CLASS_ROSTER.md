...
| 4 | **Async message delivery / act-cap** | C synchronous `write_to_buffer` in tick vs Python async dispatch (double-deliver, miss, mis-cap) | behavioral — single-delivery (XOR), per-recipient masking, first-letter cap, verified by integration tests, not a lexical bypass | enforced | INV-001 (×5 `test_inv001_*.py`) / 027 / 029; ACT-CAP-001..004. **Reclassified A→C 2026-06-02, Layer-A allowlist-guard reclaimed 2.14.116:** a *blanket* static bypass-guard is infeasible — `.messages.append` has many legitimate sites (`Character.send_to_char`, broadcast primitives, actor-self lines) so an unconditional grep false-positives. But an *allowlist* grep-guard (`tests/test_message_delivery_convention.py`, 2.14.116) forbids `.messages.append` / the two-step `getattr` bypass except for explicit chokepoints, with residual bypasses frozen as `_INV001_DEBT` (burned to empty 2.14.128). The `do_yell` hand-rolled `create_task(send_to_char)` XOR — formerly cited here as the legitimate-site that defeats a guard — was collapsed onto `push_message` (2.14.131). Contract remains Layer C for double-delivery/late ordering, now WITH a Layer-A allowlist bypass-guard + a Layer-B "review new delivery sites" element. |

| 14 | **Connection / session lifecycle (net-death link-dead)** | C `close_socket` (`src/comm.c:1075-1093`) keeps a `CON_PLAYING` char **link-dead** on socket drop — `ch->desc = NULL`, char STAYS in `char_list`/room, broadcasts `"$n has lost $s link."` + `wiznet "Net death has claimed $N"`; only a non-playing descriptor is `free_char`'d. The lingering char keeps accruing `char_update` idle timer → void@12 min → autoquit@30 min (`src/update.c:738/682`), is killable while away, and `check_reconnect` (`comm.c:1836`) **rebinds a returning player to that same in-world instance** (combat/position/transient affects preserved). Python's socket-drop path does the opposite. | behavioral / differential — sync `close_socket` (linger) vs async finally-block (immediate extract); no chokepoint. Shares the sync-teardown root cause with GL-035 (the synchronous tick cannot `await conn.close()`). | **CLOSED (2026-06-22, 2.14.211)** | **FIXED.** The socket-drop path now mirrors ROM `close_socket`: `mud/net/connection.py:_finalize_disconnect` routes a genuine link drop (not `_quit_requested`, not `_forced_disconnect`) to `_disconnect_linkdead` — `"$n has lost $s link."` room broadcast + `WIZ_LINKS` wiznet, null `desc`/`connection`, set the new `Character.link_dead` marker, release the account marker, but KEEP the char in `room` + `character_registry`. `char_update` then idles it (void@12/autoquit@30 — the INV-038/`_auto_quit_character` link-dead leg is now production-reachable), it's killable while away, and a returning player rebinds: `_select_character` → `_find_linkdead_character` returns the lingering instance (after the existing password auth — ROM `check_reconnect` is gated post-password at `nanny.c:281`), clearing `link_dead`. Explicit `do_quit` and idle-autoquit (which now sets `_quit_requested`) still fully extract. Tests: `tests/integration/test_class14_linkdead_lifecycle.py` (6 — lookup, rebind same-instance, linger/quit/forced) + `test_inv009_registry_disconnect_cleanup.py` (rewritten: link drop → one lingering link-dead entry, reconnect → same instance, no duplicate — INV-009's no-duplicate invariant preserved via the new mechanism). **Known limitation (documented, not fixed):** the rebind requires the char to be flagged `link_dead`, which is set by the *async* disconnect `finally`. A test (or pathological client) that reconnects in the SAME event-loop tick — before that teardown runs — races among three `_select_character` outcomes (takeover-prompt / rebind / fresh-load). Production is safe (the web client backs off ≥1 s before reconnecting, so teardown always wins); reconnect-flavor tests were made deterministic by issuing an explicit `quit` (save+extract) between sessions rather than relying on an abrupt close. Serializing the disconnect/login on a per-name lock is separate hardening, deliberately out of scope. Cross-ref: INV-009 (mechanism updated — pointer added), INV-038 (its link-dead idle leg is now live), GL-035 (same async-teardown root; the autoquit `_quit_requested` flag composes with it). `close_socket` remains **N/A — Networking arch** in `COMM_C_AUDIT.md` row 56 (transport plumbing); the *behavioral* contract is enforced here. |
...
1. ~~Promote class 4 to a committed Layer-A guard.~~ **Resolved differently
   (2026-06-02), then partially reclaimed (2.14.116):** the first attempt found a
   *blanket* static guard infeasible — `.messages.append` has many legitimate
   sites (actor-self lines, broadcast primitives, `Character.send_to_char`), so an
   unconditional grep false-positives. Class 4 was **reclassified A→C** (enforced
   behaviorally by INV-001/027/029 + ACT-CAP-001..004). **2.14.116** then
   reclaimed a Layer-A *allowlist* grep-guard (`tests/test_message_delivery_convention.py`):
   it forbids `<entity>.messages.append` / the two-step `getattr` bypass in `mud/`
   except for an explicit allowlist of genuine chokepoints, with the residual
   bypasses frozen as `_INV001_DEBT` and burned down to empty (2.14.128). The
   `do_yell` hand-rolled `create_task(send_to_char)` XOR — previously the canonical
   example of a legitimate site that defeats a blanket guard — was itself
   **collapsed onto `push_message`** (2.14.131), removing one more reason the guard
   needs hand-rolled exceptions.
