...
| 14 | **Connection / session lifecycle (net-death link-dead)** | C `close_socket` (`src/comm.c:1075-1093`) keeps a `CON_PLAYING` char **link-dead** on socket drop — `ch->desc = NULL`, char STAYS in `char_list`/room, broadcasts `"$n has lost $s link."` + `wiznet "Net death has claimed $N"`; only a non-playing descriptor is `free_char`'d. The lingering char keeps accruing `char_update` idle timer → void@12 min → autoquit@30 min (`src/update.c:738/682`), is killable while away, and `check_reconnect` (`comm.c:1836`) **rebinds a returning player to that same in-world instance** (combat/position/transient affects preserved). Python's socket-drop path does the opposite. | behavioral / differential — sync `close_socket` (linger) vs async finally-block (immediate extract); no chokepoint. Shares the sync-teardown root cause with GL-035 (the synchronous tick cannot `await conn.close()`). | **OPEN (verified manually 2026-06-22)** | **NO TEST — known-open divergence.** Python's genuine socket-drop (`mud/net/connection.py` non-forced `finally`, both `handle_connection*` variants) treats link loss as `do_quit`: save + INV-020 nuke_pets/die_follower + `char_from_room` + **remove from `character_registry`** + `release_account` (INV-009). Consequences verified: **(a)** `char_update` iterates `character_registry` (`game_loop.py:901`), so a removed char gets NO idle processing — the INV-038 / `_auto_quit_character` *link-dead* (`desc is None`) void@12/autoquit@30 leg is **production-unreachable dead code** (only tests like `test_inv038_idle_timer_input_reset.py` manufacture a `desc=None`-in-registry char); ROM's net-death population doesn't exist in Python. **(b)** a dropped-link PC cannot be attacked/killed during the window ROM leaves them vulnerable. **(c)** reconnect = `login_with_host`→`load_character` (reload from the autosaved pfile), NOT re-attach to a lingering instance — transient in-world state (current fight, position, non-saved affect timers) is lost. **(d)** the real socket-drop path emits no `"$n has lost $s link."` room line nor `"Net death has claimed"` wiznet; those fire only on the *forced-takeover* path (`_disconnect_session`, ROM `check_playing`), which correctly **transfers** the live char to the new descriptor. `close_socket` is marked **N/A — Networking arch** in `COMM_C_AUDIT.md` (row 56), so this was scoped out of the per-file audit; the roster is its proper home as a sync-vs-async lifecycle class. Cross-ref: INV-009 (the removal mechanism), INV-038 (the idle-timer reset it strands), GL-035 (`UPDATE_C_AUDIT.md`, same async-teardown root). Fix is a large architectural change (keep net-death chars in-world link-dead + a reconnect-rebind path + async-safe autoquit teardown) — **not yet scoped**. |
