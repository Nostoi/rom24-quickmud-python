...
- **Status**: ✅ Audited 95% — named combat-path regressions closed; May 15, 2026 XP/death branch sweep found and closed one additional real gap
- **Date**: 2026-05-15
...
`fight.c`'s currently known named combat-path regressions are closed. Keep the tracker row at 95% until a broader refresh sweep is done, but this audit doc no longer carries open implementation work. Recent verification is locked by `tests/integration/test_fight_c_do_kill_parity.py`, `tests/integration/test_fight_c_safe_room_damage_gate.py`, and `tests/integration/test_character_advancement.py::test_player_kill_applies_rom_death_penalty`.
