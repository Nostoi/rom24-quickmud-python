...
- **⚠️ Reachability caveat (post-close, advisor-prompted)**: the floor removal is ROM-faithful arithmetic, but verification of the production equip path showed `_decay_worn_light` is **effectively unreachable in live play**. `do_wear` keys worn lights under `WearLocation.HOLD` (`equipment.py:192`; `_get_wear_location` has no LIGHT branch), `Room._has_lit_light_source` looks up the str key `"0"` (`room.py:29`), and `_find_equipped_light` matches only `"light"`/int-`WearLocation.LIGHT` (`game_loop.py:357-361`) — three mutually inconsistent keys. The decay loop is also PC-only. Filed as **INV-028 candidate** (LIGHT-SLOT-KEY-COHERENCE). ARITH-202's commit stands (correct, harmless, test-pinned); its production effect is gated by INV-028.
...
0. **INV-028 candidate — LIGHT-SLOT-KEY-COHERENCE (filed this session, NOT enforced).** The worn-light slot is keyed three inconsistent ways: `do_wear` → `WearLocation.HOLD` (int), `Room._has_lit_light_source` → str `"0"`, `_find_equipped_light` → `"light"`/int-`WearLocation.LIGHT`. Net effect: PC light sources never burn out and PC-held lights are mis-tracked in room lighting vs ROM. Full prose + proposed enforcement + regression filename in `docs/parity/CROSS_FILE_INVARIANTS_TRACKER.md` (INV-028 candidate block). This is the highest-value follow-up surfaced this session — closing it makes ARITH-202 actually take effect in play.
