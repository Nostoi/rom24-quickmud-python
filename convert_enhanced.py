...
def main():
    ...
    print(f"Loaded {len(area_registry)} areas, {len(room_registry)} rooms, {len(mob_registry)} mobs, {len(obj_registry)} objects")
    ...
    for area_vnum, area in area_registry.items():
        ...
        print(f"Converted {area.name} -> {output_file} ({len(area_rooms)} rooms, {len(area_mobs)} mobs, {len(area_objects)} objects)")
