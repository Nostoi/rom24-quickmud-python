...
- **GL-034 — idle autoquit now staggers one player per tick (ROM `ch_quit` semantics).** ROM `char_update` (`src/update.c:682-683`) sets `ch_quit = ch` for each char with pre-increment `timer > 30`, so the single pointer ends the loop holding the *last* such char, and the second loop quits only that one (`:897-900`) — at most one idle autoquit per tick. The Python port collected every `timer > 30` char into a list and quit them all the same tick. Fix: `char_update` tracks a single `autoquit_candidate` (last-wins overwrite) and quits at most that one. Practical impact is small (needs two PCs idle ≥31 ticks simultaneously) but it is a genuine divergence. With GL-035 the candidate population is now ROM-faithful regardless of link state. Test: `tests/integration/test_inv038_idle_timer_input_reset.py::test_only_last_idle_player_autoquits_per_tick`.
