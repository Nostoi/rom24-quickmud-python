...
- `WIZ-010` — `invis` and `incognito` now broadcast room-wide `act()` messages (`"$n slowly fades into thin air."` / `"$n slowly fades back into existence."` / `"$n cloaks $s presence."`) per ROM `src/act_wiz.c:4329-4420`. `incognito` clears `reply` when setting a specific level. Added `\n\r` line endings.
