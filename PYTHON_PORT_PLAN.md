...
<!-- NEXT-ACTIONS-START -->
- movement_encumbrance: [P0] Restore ROM carry capacity overrides for immortals and pets and reuse Character.get_curr_stat in can_carry_* helpers
<!-- NEXT-ACTIONS-END -->
...
### movement_encumbrance — Parity Audit 2025-10-06

STATUS: completion:❌ implementation:partial correctness:fails (confidence 0.40)
KEY RISKS: side_effects, lag_wait
TASKS:
- [P0] Restore ROM carry capacity overrides for immortals and pets and use `Character.get_curr_stat` in `can_carry_n`/`can_carry_w`.
  EVIDENCE: C src/handler.c:900-934 (`can_carry_n`/`can_carry_w` grant immortals massive capacity and zero pets)
  EVIDENCE: PY mud/world/movement.py:35-75 (`_get_curr_stat` ignores `mod_stat` and never checks immortal/pet flags)
  EVIDENCE: PY mud/models/character.py:145-188 (`get_curr_stat` already clamps perm+mod stats but `move_character` bypasses it)
  RATIONALE: The port leaves immortals capped at mortal limits and allows charmed pets to haul gear, breaking ROM encumbrance parity and making STR/DEX buffs meaningless.
  FILES: mud/world/movement.py; mud/models/character.py; tests/test_movement_encumbrance.py
  TESTS: pytest -q tests/test_movement_encumbrance.py::test_carry_capacity_matches_rom (new)
  ACCEPTANCE: Immortal players report 1000 item slots/10,000,000 weight capacity, pets return zero allowance, and STR/DEX modifiers via `mod_stat` shift limits like ROM.
  REFERENCES: C src/handler.c:900-934; PY mud/world/movement.py:35-75; PY mud/models/character.py:145-188
  ESTIMATE: M; RISK: side_effects
...
NOTES:
- C: src/handler.c:900-934 defines ROM carry capacity caps for immortals and pets; src/act_move.c:69-214 enforces visibility and stealth messaging order.
- PY: mud/world/movement.py:35-300 omits immortal/pet overrides in `can_carry_*` and handles movement visibility/broadcasting; mud/models/character.py:145-188 already exposes `get_curr_stat` needed for parity.
- Tests: Existing suites cover visibility, auto-look, and mobprog entry triggers; new coverage must add immortal/pet encumbrance assertions.
- Applied tiny fix: none
...
### resets — Parity Audit 2025-10-06

STATUS: completion:❌ implementation:partial correctness:fails (confidence 0.33)
KEY RISKS: side_effects, flags
TASKS:
- ✅ [P0] Restore ROM object level selection for `'G'`/`'E'` resets — done 2025-09-21
  EVIDENCE: PY mud/spawning/reset_handler.py:L518-L566; PY mud/spawning/obj_spawner.py:L8-L26
  EVIDENCE: PY mud/utils/rng_mm.py:L116-L124; TEST tests/test_spawning.py::test_shopkeeper_inventory_levels_match_rom
- ✅ [P0] Mirror ROM `number_fuzzy(LastObj->level)` propagation for `'P'` resets so nested contents inherit container levels and respect global limits. — done 2025-09-21
  EVIDENCE: PY mud/spawning/reset_handler.py:L568-L648; PY mud/utils/rng_mm.py:L116-L124
  EVIDENCE: TEST tests/test_spawning.py::test_container_resets_preserve_item_levels
...
- ✅ [P0] Maintain Area.nplayer/empty parity on char moves to support forced reset timers — done 2025-10-02
  ...
  ESTIMATE: M; RISK: medium
NOTES:
- C: src/db.c:1788-1967 documents `reset_room` level calculations for `'P'`, `'G'`, and `'E'` commands, including `number_fuzzy` usage and shopkeeper inventory scaling.
- PY: mud/spawning/reset_handler.py:145-470 leaves `_compute_object_level` stubbed and spawns level-0 contents for `'P'` resets; mud/spawning/obj_spawner.py:8-19 never applies the requested level to new objects.
- DOC/ARE: doc/area.txt:395-438 explains reset command parameters, while area/midgaard.are:6084-6160 demonstrates potion shop and donation pit resets that rely on leveled loot.
- Applied tiny fix: none
